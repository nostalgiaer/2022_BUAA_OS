...
void fork(struct Env *e) {
	...
	child->pre_brother = NULL;
	child->next_brother = NULL;

/*	if (e->first_child == NULL) {
		e->first_child = child;
		e->last_child = child;
	} else {
		e->last_child->next_brother = child;
		child->pre_brother = e->last_child;
		e->last_child = child;
	}
*/
...
int get_child_sum(u_int envid) {
	...
	int cnt = 0;
	struct Env *child = e->first_child;

	while (child != NULL) {
		cnt += (1 + get_child_sum(child->env_id));
...
void append_child(struct Env *father, struct Env *child) {
	if (father->first_child == NULL) {
		father->first_child = child;
	} 

	if (father->last_child != NULL) {
		father->last_child->next_brother = child;
		child->pre_brother = father->last_child;
	}
	
	father->last_child = child;
	return ;
}
...
void kill(u_int envid) {
	...
	while (root->env_parent_id != 0) {
		envid2env(root->env_parent_id, &root, 0);
	}

	if (e->pre_brother != NULL) {
		e->pre_brother->next_brother = e->next_brother;
	}
	if (e->next_brother != NULL) {
		e->next_brother->pre_brother = e->pre_brother;
	}

	struct Env *father;
	envid2env(e->env_parent_id, &father, 0);
	if (father->first_child == e) {
		father->first_child = e->next_brother;
	}
	...
	if (e->first_child != NULL) {
		...
		while (child->next_brother != NULL) {
			envid2env(child->next_brother->env_id, &child, 0);
			append_child(root, child);
		}
	}
