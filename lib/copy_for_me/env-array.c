...
void fork(struct Env *e) {
	struct Env *child;
	
	env_alloc(&child, e->env_id);
	e->childs[e->child_number] = child->env_id;
	e->child_number++;
	
	return ;
}

int get_child_sum(u_int envid) {
	struct Env *e;
	envid2env(envid, &e, 0);
	if (e -> child_number == 0) {
		return 0;
	...
	int cnt = 0, i;
	struct Env *child;
	for (i = 0; i < e->child_number; i++) {
		cnt += (1 + get_child_sum(e->childs[i]));
	}

	return cnt;
}

void output(u_int envid) {
	struct Env *e;
	envid2env(envid, &e, 0);

	int first_child_id;
	if (e->child_number > 0) {
		first_child_id = ENVX(e->childs[0]);
	} else {
		first_child_id = 0;
	}

	struct Env *father;
	envid2env(e->env_parent_id, &father, 0);
	int i;
	for (i = 0; i < father->child_number; i++) {
		if (father->childs[i] == envid) {
			break;
		}
	}
	
	int prev_bro_id = (i == 0)? 0: ENVX(father->childs[i-1]);
	int next_bro_id = (i == father->child_number)? 0: ENVX(father->childs[i+1]);

	printf("current %d, first child id is %d, prev bro id is %d, next bro id is %d\n", ENVX(envid), first_child_id, prev_bro_id, next_bro_id);
	return ;
}
...
void kill(u_int envid) {
	...
	envid2env(envid, &e, 0);
	temp = e;
	while (temp->env_parent_id) {
		envid2env(temp->env_parent_id, &temp, 0);
	}
	root = temp;	
	...
	for (i = 0; i < father->child_number; i++) {
		if (father->childs[i] == envid) {
			for (j = i; j < father->child_number-1; j++) {
	...
	for (i = 0; i < e->child_number; i++) {
		root->childs[root->child_number] = e->childs[i];
		root->child_number++;
	}
...
u_int check_same_father(u_int envid1, u_int envid2) {
	int stack[100], cnt = 0;
	struct Env *e1;
	struct Env *e2;
	...
	while (e1->env_parent_id) {
		envid2env(e1->env_parent_id, &e1, 0);
		stack[cnt++] = e1->env_id;
	}	
	int i;

	while (e2->env_parent_id) {
		envid2env(e2->env_parent_id, &e2, 0);
		for (i = 0; i < cnt; i++) {
			if (e2->env_id == stack[i]) {
				return e2->env_id;
			}
		} 
	}
