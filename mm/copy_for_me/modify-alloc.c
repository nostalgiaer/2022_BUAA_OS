...
*/
	...
	int unused_page_number = PPN(PADDR(freemem));
	int i = 0;

//	LIST_INSERT_HEAD(&page_free_list, &pages[0], pp_link);
//	LIST_INSERT_HEAD(&page_free_list, &pages[1], pp_link);
...
//	printf("%d\n", i);
	...
	for (i = 0; i < unused_page_number; i++) {
		pages[i].pp_ref = 0;
		LIST_INSERT_HEAD(&page_free_list, &pages[i], pp_link);
	}

	for (i = unused_page_number; i < npage; i++) {
		pages[i].pp_ref = 1;
	}
