...
void page_init(void)
{
	 ...
	 * filed to 1) */
	int used_page_number = PPN(PADDR(freemem));
	int i;

/*	for (i = 0; i < used_page_number; i++) {
		pages[i].pp_ref = 1;
	}
*/
	/* Step 4: Mark the other memory as free. */
/*	for (i = used_page_number; i < npage; i++) {
		LIST_INSERT_HEAD(&page_free_list, &pages[i], pp_link);
		pages[i].pp_ref = 0;
	}
*/
	int page_group_number = used_page_number / 32;
	int individual_page_number = used_page_number % 32;

	for (i = 0; i < MAPSIZE; i++) {
		bitmap[i] = 0;
	}

	for (i = 0; i < page_group_number; i++) {
		bitmap[i] = ~0;
	}

	for (i = 0; i < individual_page_number; i++) {
		bitmap[page_group_number] |= (1 << i);
	}

	printf("%b\n", bitmap[page_group_number]);
}
...
void page_free(struct Page *pp)
{
	...
	if (pp -> pp_ref == 0) {
		...
		int page_number = page2ppn(pp);
		int page_group_number = page_number / 32;
		int individual_page_number = page_number % 32;
		bitmap[page_group_number] &= ~(1 << individual_page_number);
