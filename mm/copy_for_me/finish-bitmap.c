...
*/
	...
	unsigned int page_group = used_page_number / 32;
	unsigned int page_individual = used_page_number % 32;
	int j;

	for (i = 0; i < page_group; i++) {
		bitmap[i] = ~0;
		for (j = 0; j < 32; j++) {
			pages[i * 32 + j].pp_ref = 1;
		}
	} 

	for (j = 0; j < page_individual; j++) {
		bitmap[page_group] |= (1 << j);
		pages[page_group * 32 + j].pp_ref = 1;
	}
...
//		LIST_INSERT_HEAD(&page_free_list, pp, pp_link);	
		...
		int page_group = page_number / 32;
		int page_individual = page_number % 32;
		int i;
		
		bitmap[page_group] &= (~(1 << page_individual));
		return ;
	}
